...
with tf.variable_scope('loss'):
    discriminator_loss = -(tf.reduce_mean(tf.log(real_prediction) + 1e-3) + tf.reduce_mean(tf.log(1 - fake_prediction + 1e-3)))
    fooled_rate = tf.reduce_mean(tf.to_float(fake_prediction > 0.5))
    accuracy = (tf.reduce_mean(tf.to_float(real_prediction > 0.5)) + tf.reduce_mean(tf.to_float(fake_prediction < 0.5)))/2
    generative_loss = -tf.reduce_mean(tf.log(fake_prediction))

    # discriminator_loss = -(tf.reduce_mean(tf.log(prediction[:REAL_COUNT, :]) + 1e-3) + tf.reduce_mean(tf.log(1 - prediction[REAL_COUNT:, :] + 1e-3)))
    # fooled_rate = tf.reduce_mean(tf.to_float(prediction[REAL_COUNT:, :] > 0.5))
    # accuracy = tf.reduce_mean(tf.to_float(tf.equal(tf.to_float(prediction > 0.5), discriminator_labels)))
    # generative_loss = -tf.reduce_mean(tf.log(prediction))
    discriminator_summ = tf.summary.merge(
        [tf.summary.scalar('discriminator_loss', discriminator_loss), tf.summary.scalar('accuracy', accuracy),tf.summary.scalar('fooled_rate', fooled_rate)])
...
with tf.variable_scope('train'):
    ...
    discriminator_train_step = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(discriminator_loss, var_list=discriminator_variables)
    # generative_train_step = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(generative_loss, var_list=generator_variables)
    generative_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
                    ...
                    _, summ, grad_summ = sess.run((generative_train_step, generator_summ, gradient_summ),
                                                  feed_dict={random_sample: np.random.normal(0, 1,
                                                                                             size=(FAKE_COUNT,
                                                                                                   SAMPLE_SPACE_DIM))})
                    writer.add_summary(summ, step)
                    writer.add_summary(grad_summ, step)

            if step % 50 == 0:
                summ = sess.run(img_summ, feed_dict={
                    random_sample: z})
                writer.add_summary(summ, step)
            if step % 100 == 0:
